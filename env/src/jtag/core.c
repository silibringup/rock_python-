...
// extern char global_tdo_value[32];

// hack: replace with the larger space for long scan chain
extern char  global_tdo_value[1024];
extern uint8_t global_tdo[1024/2];
int hex2int(char ch);
			...
			case JTAG_SCAN:
				...
				for (int i = 0; i < cmd->cmd.scan->num_fields; i++) {
					...
					if (field->in_value) {
						...
						// LOG_USER("  %db  scan TDO: %s", field->num_bits, str);
						// LOG_DEBUG_IO("  %db  in: %s", field->num_bits, str);

						for(size_t ii=0;ii<strlen(str); ii++){
							global_tdo_value[ii] = str[ii];
						}

						int n_bit2maskout = (field->num_bits%8)? 8-field->num_bits%8 : 0;
						int n_byte = (field->num_bits+7)/8;
						memset(global_tdo, 0, 1024/2);
						memcpy(global_tdo, field->in_value, n_byte);
						global_tdo[n_byte-1] &= (0xff >> n_bit2maskout);

						// // TO SHOW TDO VALUE IN BYTES IN DEBUGGING DETAIL
						// for(int kk=0;kk<n_byte;kk++){
						// 	LOG_USER("    scan tdo value[%d] before format: %02x", kk, global_tdo[kk]);	
						// }
