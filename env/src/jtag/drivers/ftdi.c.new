...
extern int scan_num_bits;
extern uint8_t* scan_values;
extern struct mpsse_ctx *mpsse_ctx_hack;
extern uint8_t ftdi_jtag_mode_hack;
extern char value_str[65536];
...
{
	if(mpsse_ctx_hack == NULL){
		mpsse_ctx_hack = mpsse_ctx;
	}
	ftdi_jtag_mode_hack = ftdi_jtag_mode;
	...
	for (int i = 0; i < cmd->cmd.scan->num_fields; i++, field++) {
		...
		field->num_bits = scan_num_bits;

		// dump_to_hex(scan_values, (field->num_bits+7)/8);
		// LOG_USER("field %d, num bits = %d, 0x%s", i, field->num_bits, value_str, true);
		

		// hack: irscan has no TDO sampled out. add such an ability to openocd
		if(cmd->cmd.scan->ir_scan){
			void *t = malloc(DIV_ROUND_UP(field->num_bits, 8));
			field->in_value = t;
		}
		// LOG_USER("%s %s%s field %d/%d %d bits",
		...
		if (i == cmd->cmd.scan->num_fields - 1 && tap_get_state() != tap_get_end_state()) {
			...
			bit_copy(&last_bit, 0, scan_values, field->num_bits - 1, 1);
		...
		// // hack: add 1000 cycle trailing idle
		// {
