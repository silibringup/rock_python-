...
class Rock_reg():
    ...
    def debug_write(self,value=None,check_ack=True,*args, **kw): # *args priv etc, **kw field settings
        self.helper.pdebug(f"Write {self.name} {value}")
        addr  = self.block.base + self.offset 

        if value is not None:
            self.helper.j2h_write(addr,value,check_ack)
        else :
            for field in kw:
                searched = 0
                for field_obj in self.field_list:
    ...
    def debug_read(self,check_ack=True,*args): #*args priv etc
        addr  = self.block.base + self.offset
        value = self.helper.j2h_read(addr,check_ack)
        self.set_field_from_value(value)
        res = DotDict()
        for field_obj in self.field_list:
            res[field_obj['name']] = field_obj['value']
    ...
    def debug_update(self,*args, **kwargs): # *args priv etc
        rd_obj = self.debug_read(*args)
        ...
        for fld in kwargs:
            if fld in rd_obj:
                ...
                self.helper.perror("Not able to find %s in Register %s" % (fld, self.name))
        self.debug_write(*args, **new_kwargs) 
    ...
    def debug_poll(self, timeout=100000, interval=0, *args, **kwargs):
        count = 0
        while count < timeout:
            rd_obj = self.debug_read(*args)
