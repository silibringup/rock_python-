...
with Test(sys.argv) as t:
    ...
    def flash_cs_switch(bm, flash_num):
        bm.bmon_cfg_0.update(ap_flash_cs_sel=flash_num)
    ...
    def validate_ap_access_flash(ap_id,bm_cs,monitor):
        ...
        if read_value_1 == write_data:
            helper.perror("Flash not changed")
        ...
        if read_id_2_value != read_id_0_value:
            helper.perror("Flash not changed back")
        ...
        LOG(f"legal 1-1-1 read data from Flash address 0x01_2300 - 0x01_2307 : {hex(read_value_2)}")
        if read_value_2 != write_data:
            helper.perror("Flash not changed")
