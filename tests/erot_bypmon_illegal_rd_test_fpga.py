...
with Test(sys.argv) as t:
    ...
    read_bytes   = []
    read_value   = 0
    write_data_0 = 0xf1e2d3c4
    write_data_1 = 0xc4c4a5a5
    default_data = 0xffffffffffffffff
    addr_list_0  = [0x1f,0xff,0xfc]
    addr_list_1  = [0x20,0x00,0x00]
    ...
    def config_bm_filter(bm):
        bm.mem_permission_0.update(region_0=7,region_1=2)

        #all_access for addr(0x0-0x10_0000)
        bm.reg0_start_address_0.write(0x0)
        bm.reg0_end_address_0.write(0x200)

        #write_only for addr(0x20_0000-0x30_0000)
        ...
        bm.cmd_filter_cmd_4_0.update(dis_cmd=3,cmd_type=1,addr_phase=1,data_io_mode=1,cs_term=1,dum_cyc_en=1,instr_4b=0,cmd_log=3,instr_code=0x0b,valid=1)
        #1-1-2 read opcode, illegal rcv
        ...
        bm.cmd_filter_cmd_5_0.update(dis_cmd=3,cmd_type=1,addr_phase=1,data_io_mode=2,cs_term=1,dum_cyc_en=1,instr_4b=0,cmd_log=3,instr_code=0x3b,valid=1)
        #1-1-4 read opcode, illegal rcv
        ...
        bm.cmd_filter_cmd_6_0.update(dis_cmd=3,cmd_type=1,addr_phase=1,data_io_mode=3,cs_term=1,dum_cyc_en=1,instr_4b=0,cmd_log=3,instr_code=0x6b,valid=1)
        #1-0-1 read config 
        ...
        #1-1-1 slow read with no dummy_cycle, illegal rcv
        ...
        bm.cmd_filter_cmd_10_0.update(dis_cmd=3,cmd_type=1,addr_phase=1,data_io_mode=1,cs_term=1,dum_cyc_en=0,instr_4b=0,cmd_log=3,instr_code=0x03,valid=1)
        #1-0-1 read ID
    ...
    def validate_ap_access_flash(ap_id,bm_cs,monitor):
        ...
        #Enable VIP to send legal memory wr: 0x1f_fffc - 0x1f_ffff(4 byte)
        #1-1-1 write
        helper.spi_write(spi_port=ap_id, cs_id=bm_cs, 
                         n_instruction_lane=1, n_instruction_bits=8, instruction=[0x02],
                         n_address_lane=1, n_address_bits=24, address=addr_list_0, 
                         n_data_lane=1, data=list(write_data_0.to_bytes(4,"big")))
        ap_wait_flash_write_done(ap_id,bm_cs)
        LOG(f"1-1-1 write data to legal Flash address 0x1f_fffc - 0x1f_ffff: {hex(write_data_0)}")
        ...
        #Enable VIP to send legal memory wr: 0x20_0000 - 0x20_0003(4 byte)
        #1-1-1 write
        helper.spi_write(spi_port=ap_id, cs_id=bm_cs, 
                         n_instruction_lane=1, n_instruction_bits=8, instruction=[0x02],
                         n_address_lane=1, n_address_bits=24, address=addr_list_1, 
                         n_data_lane=1, data=list(write_data_1.to_bytes(4,"big")))
        ap_wait_flash_write_done(ap_id,bm_cs)
        LOG(f"1-1-1 write data to legal Flash address 0x20_0000 - 0x20_0003: {hex(write_data_1)}")


        ########################################################################
        ####################### 1-1-1 ILLEGAL RD (RCV) #########################
        ########################################################################
        LOG(f"##################################################################")
        LOG(f"#################### 1-1-1 ILLEGAL RD (RCV) ######################")
        LOG(f"##################################################################")
        #Enable VIP to send illegal memory rd: 0x20_0000 - 0x20_0003(4 byte)
        #1-1-1 read
        helper.wait_sim_time("us", 1)
        read_bytes_0 = helper.spi_read(spi_port=ap_id, cs_id=bm_cs, 
                       n_instruction_lane=1, n_instruction_bits=8, instruction=[0x0b],
                       n_address_lane=1, n_address_bits=24, address=addr_list_1, 
                       n_data_lane=1, nbr_rd_bytes=4, dummy_cycles=dummy_cycle) 
        read_value_0 = int.from_bytes(read_bytes_0, "big")
        LOG(f"1-1-1 read data from illegal Flash address 0x20_0000 - 0x20_0003: {hex(read_value_0)}")
        if read_value_0 != write_data_1:
        ...
        if read_id_0_value != read_id_value:
            helper.perror("BYP_MON encounter NON_RCV error")


        ########################################################################
        ####################### 1-1-2 ILLEGAL RD (RCV) #########################
        ########################################################################
        LOG(f"##################################################################")
        LOG(f"#################### 1-1-2 ILLEGAL RD (RCV) ######################")
        LOG(f"##################################################################")
        #Enable VIP to send illegal memory rd: 0x20_0000 - 0x20_0003(4 byte)
        #1-1-2 read
        helper.wait_sim_time("us", 1)
        read_bytes_1 = helper.spi_read(spi_port=ap_id, cs_id=bm_cs, 
                       n_instruction_lane=1, n_instruction_bits=8, instruction=[0x3b],
                       n_address_lane=1, n_address_bits=24, address=addr_list_1, 
                       n_data_lane=2, nbr_rd_bytes=4, dummy_cycles=dummy_cycle) 
        read_value_1 = int.from_bytes(read_bytes_1, "big")
        LOG(f"1-1-2 read data from illegal Flash address 0x20_0000 - 0x20_0003: {hex(read_value_1)}")
        if read_value_1 != write_data_1:
            LOG(f"----- BYP_MON enter RCV state -----")
            LOG(f"----- BYP_MON exit RCV state -----")
        ...
        if read_id_1_value != read_id_value:
            helper.perror("BYP_MON encounter NON_RCV error")


        ########################################################################
        ####################### 1-1-4 ILLEGAL RD (RCV) #########################
        ########################################################################
        LOG(f"##################################################################")
        LOG(f"#################### 1-1-4 ILLEGAL RD (RCV) ######################")
        LOG(f"##################################################################")
        #Enable VIP to send illegal memory rd: 0x20_0000 - 0x20_0003(4 byte)
        #1-1-4 read
        helper.wait_sim_time("us", 1)
        read_bytes_2 = helper.spi_read(spi_port=ap_id, cs_id=bm_cs, 
                       n_instruction_lane=1, n_instruction_bits=8, instruction=[0x6b],
                       n_address_lane=1, n_address_bits=24, address=addr_list_1, 
                       n_data_lane=4, nbr_rd_bytes=4, dummy_cycles=dummy_cycle) 
        read_value_2 = int.from_bytes(read_bytes_2, "big")
        LOG(f"1-1-4 read data from illegal Flash address 0x20_0000 - 0x20_0003: {hex(read_value_2)}")
        ...
        if read_id_2_value != read_id_value:
            helper.perror("BYP_MON encounter NON_RCV error")


        ########################################################################
        ##################### 1-1-1 ILLEGAL SLOW RD (RCV) ######################
        ########################################################################
        LOG(f"##################################################################")
        LOG(f"################## 1-1-1 ILLEGAL SLOW RD (RCV) ###################")
        LOG(f"##################################################################")
        #Enable VIP to send legal memory rd: 0x1f_fffc - 0x20_0003(8 byte)
        #1-1-1 slow read
        helper.wait_sim_time("us", 1)
        ...
        LOG(f"1-1-1 read data from illegal Flash address 0x1f_fffc - 0x20_0003 : {hex(read_value_3)}")
        read_value_3_low_addr = read_value_3 >> 32
        read_value_3_high_addr = read_value_3 & 0xffffffff
        LOG(f"Low addr 4 byte : {hex(read_value_3_low_addr)}")
        LOG(f"High addr 4 byte : {hex(read_value_3_high_addr)}")
        if read_value_3_low_addr != write_data_0:
            helper.perror("BYP_MON block legal rd")
        if read_value_3_high_addr == write_data_1:
            helper.perror("receive data from illegal address, 1-1-1 RCV illegal memory read not blocked")
        else:
            LOG(f"----- BYP_MON enter RCV state -----")
            LOG(f"----- BYP_MON exit RCV state -----")
    ...
    LOG("START BYPMON_ILLEGAL_RD FPGA TEST")
