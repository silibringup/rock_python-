...
with Test(sys.argv) as t:
    ...
    def setup_transmit(spi):
        #spi.COMMAND2_0.update(SI_RM_DUAL_SPI_ENABLE=1)#no dummy
        spi.COMMAND2_0.update(SI_RM_DUMMY_CYC_LENGTH=8, SI_RM_DUAL_SPI_ENABLE=1,SI_RM_CMD_BIT_LENGTH=7, SI_RM_ADDRESS_BIT_LENGTH=23)#no dummy
    ...
    def check_tx_data(spi,cs):
        read_bytes = helper.spi_read(spi_port=spi, cs_id=cs, n_data_lane=2, nbr_rd_bytes=2,n_instruction_lane=1, n_instruction_bits=8, instruction=[0xf0], n_address_lane=1, n_address_bits=24, address=[0x55,0xff,0x55],dummy_cycles=8)
        read_value = int.from_bytes(read_bytes, "big")
        helper.pinfo(f'read_value = {read_value}')
