...
with Test(sys.argv) as t:
    ...
    def validate_instr_4b(ap_id,bm_cs,monitor):
        ########################################################################
        ############################### INSTR_4B ###############################
        ########################################################################
        LOG(f"##################################################################")
        ...
        #Enable VIP to send legal cfg: 4B entry
        #1-0-0 config
        helper.spi_write(spi_port=ap_id, cs_id=bm_cs, 
                     n_instruction_lane=1, n_instruction_bits=0, instruction=[],
                     n_address_lane=1, n_address_bits=0, address=[], 
                     n_data_lane=1, data=[0xb7])
        LOG(f"legal 1-0-0 4B entry to Flash")

        #Poll ADDR_MODE_4B_INT=1'b1
        monitor.addr_mode_4b_int_0.poll(val=1)
        helper.wait_sim_time("us", 1)

        #BYP_MON SW rst & re-init
        ...
        monitor.addr_mode_4b_int_0.poll(val=0)
        ini_bm(monitor)
        helper.wait_sim_time("us", 15)
        LOG(f"In terms of 4B mode: BYP_MON out, Flash in")
        ...
        #1-1-1 4B write
        helper.spi_write(spi_port=ap_id, cs_id=bm_cs, 
                     n_instruction_lane=1, n_instruction_bits=8, instruction=[0x12],
                     n_address_lane=1, n_address_bits=32, address=addr_list_1, 
        ...
        read_bytes_1 = helper.spi_read(spi_port=ap_id, cs_id=bm_cs, 
                     n_instruction_lane=1, n_instruction_bits=8, instruction=[0x13],
                     n_address_lane=1, n_address_bits=32, address=addr_list_2, 
                     n_data_lane=1, nbr_rd_bytes=8, dummy_cycles=0) 
        read_value_1 = int.from_bytes(read_bytes_1, "big")
        LOG(f"legal 1-1-1 read data from Flash address 0x01_2300 - 0x01_2307 : {hex(read_value_1)}")
    ...
    def validate_ap_socv(qspi,monitor,ap,cs):
        ...
        validate_instr_4b(ap,cs,monitor)
