...
class PrgnRiscV:
    ...
    def init_imem_zero(self,has_parity=True):
        mem_size = 512*1024
        deposit_list = []
        for i in range(mem_size):
            addr = i + self.imem_pa_start
            deposit_list.append(self.deposit_mem_byte(addr,0,has_parity))
        if not self.helper.loadimem_frontdoor:
            self.batch_deposit(deposit_list, f'{self.hdl_root}.imem.dep_zero')
...
class FSP(PrgnRiscV):
    ...
    def __init__(self, hdl_proxy=None, hdl_root=None,
                 imem_pa_start=0x0100000, imem_size=0x80000,
                 dmem_pa_start=0x0180000, dmem_size=0x80000,
                 memory_model="SIM"):
