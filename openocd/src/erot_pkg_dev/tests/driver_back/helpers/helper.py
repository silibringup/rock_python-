...
class Helper:
    ...
    def j2h_unlock(self):
        #TODO@changliu: to refine at FNL
    ...
    def backdoor_fill_mram(self, binfile, start_entry_offset, wanna_corrupt=False, only_peek=False):
        ...
        def deposit_mram_entry(entry_addr, data256):
            for j in range(256 // 32):
                ...
                if Helper.target == "simv_fpga":
                    self.perror("No MRAM hdl path provided for FPGA RTL. Need to add it later.")
                else:
                    hdl_path = f"ntb_top.u_nv_top.u_sra_sys0.u_l1_cluster.u_NV_nverot_mwrap.u_mram_sys.mram_inst.core.main_mem[{entry_addr}][{(j + 1) * 32 - 1}:{j * 32}]"
    ...
    def backdoor_fill_fuse_macro(self, macro_idx, data):
        if Helper.target == "simv_fpga":
            self.perror("No fuse ram hdl path provided for FPGA RTL. Need to add it later.")
        else:
            for i in range(len(data)):
                hdl_path = f"ntb_top.u_nv_top.u_sra_sys0.u_l1_cluster.u_NV_fuse.u_fuse_macro_wrap.u_fuse_macro_{macro_idx}.fuse_data[{i}]"
...
"""
            f.write(txt)
        subprocess.call(["nvrun", "gen_brom_nop_block_index", "-pj", "sr01"])
    ...
    def fuse_attestation_init(self, keystore_bin):
        ...
        def place_row_data(row, int32):
            ...
            for n, b in enumerate(b32):
                hdl_path = f"ntb_top.u_nv_top.u_sra_sys0.u_l1_cluster.u_NV_fuse.u_fuse_macro_wrap.u_fuse_macro_{macro}.fuse_data[{b_offset+31-n}]"
    ...
    def generate_boot_image(self, is_mfg=False, is_attestation=False, opt_secure_fsp_debug_dis=0, opt_production_mode=0):
        import sys
